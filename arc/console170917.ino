...
void getMETAR () {        // ================= GET METAR ================== //
  ...
  if (client.connect(server, 80)) {
    client.println("GET /data/METAR/KPDK.1.txt HTTP/1.1");
...
// METAR KPDK 172253Z 36003KT 10SM CLR 28/18 A3005 RMK AO2 SLP162 T02830178
  ...
  int cchour, AorP;
  chour += UTCOffset;
  if (chour < 0)        { chour += 24; cday -= 1; }
  else if (chour > 23)  { chour -= 24; cday -= 1; }
  else if (chour == 0)  { chour += 12; }

  convertTime(chour, &cchour, &AorP);
  char agePretty[50];
  sprintf(agePretty, "%d:%02d%s", cchour, cminute, ampm[AorP]);
  ...
  char tempStr[40]; sprintf(tempStr, "%s°F",  temp_d);  
  ...
  char baroStr[40]; sprintf(baroStr, "%s""hg", baro_d);
  ...
  tft.setCursor(MARGIN, LineNu[firstl]);
  tft.print("Currents at ");
  tft.print(agePretty);
  tft.fillRect(tft.width() / 2, LineNu[firstl] -10, tft.width() / 2, 
                                            10, GREEN);
  
  tft.setCursor(MARGIN, LineNu[firstl + 1]);
  tft.print(textStr);

  tft.setCursor(MARGIN, LineNu[firstl + 2]);
  tft.print("Humidity");
  tft.setCursor(MARGIN + TAB, LineNu[firstl + 2]);
  tft.print(rHumStr);

  tft.setCursor((tft.width() / 2), LineNu[firstl + 2]);
  tft.println("Dew Point");
  tft.setCursor((tft.width() / 2) + TAB, LineNu[firstl + 2]);
  tft.print(dwptStr);

//  tft.setCursor(MARGIN, LineNu[4]);
//  tft.print("Heat Index");
//  tft.setCursor(MARGIN + TAB, LineNu[4]);
//  tft.print(htIxStr);

  tft.setCursor(MARGIN, LineNu[firstl + 3]);
  tft.print("Wind");
  tft.setCursor(MARGIN + TAB, LineNu[firstl + 3]);
  tft.print(windStr);
  
  tft.setCursor((tft.width() / 2), LineNu[firstl + 3]);
  tft.println("Barometer");
  tft.setCursor((tft.width() / 2) + TAB, LineNu[firstl + 3]);
  tft.print(baroStr); 
  ...
  char TimeStamp[50];
  sprintf(TimeStamp, "%d:%02d%s", cchour, rtc.getMinutes(), ampm[AorP]);
  ...
  Serial.print(dwptStr);    Serial.print("\t");
  Serial.print(baroStr);    Serial.print("\t");
  Serial.print(windStr);    Serial.print("\t");
